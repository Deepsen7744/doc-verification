...
export default function AppContextProvider({ children }) {
  ...
  const [contractAddress, setContractAddress] = useState(
    '0x8983c4aB71c50203273D5B804721A3Cf36F2Fe8e'
  )
  ...
  async function getEnrolledStudentsInCourse(_instituteAddress, _courseName) {
    try {
      const data = await contract.getEnrolledStudentsInCourse(
        _instituteAddress,
        _courseName
      )
      console.log(data)
  ...
  async function createCertificate(
    ...
    try {
      await contract.createCertificate(
        _studentAddress,
        _instituteAddress,
        _courseName,
        _transactionHash
  ...
  async function uploadCertificate(
    ...
    try {
      await contract.uploadCertificate(
        _studentAddress,
        _instituteAddress,
        _transactionHash,
        _courseName,
